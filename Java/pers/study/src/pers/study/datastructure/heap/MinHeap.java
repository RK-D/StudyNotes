...
public class MinHeap<E extends Comparable<E>> {
    ...
    /**下沉元素
     * 传入索引 e ,选择左右孩子中最小的孩子结点比较，若大于孩子结点，交换元素
    ...
    public void siftDown(int e){
        while (leftChild(e) < data.getSize()){
            ...
            if (y + 1 < data.getSize() && data.get(y + 1).compareTo(data.get(y)) < 0){
                //data[y] 是 leftChild、 rightChild 的Min
                y = rightChild(e);
            }
            //比较传入的索引 e 的值是否比，左右孩子结点值小
    ...
    /**replace操作： 取出最小元素后，放入一个新元素
     * 实现：
     * 1. 实现extractMin ，再add ，两次O（log n）
     ...
     * 3.比较k和他的叶子结点，是否需要siftDown ，（和最小的子节点交换一下就行）
     * */
